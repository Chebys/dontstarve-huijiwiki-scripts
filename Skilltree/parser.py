...
class LuaParser:
    def __init__(self, builtins: Optional[Dict[str, Any]] = None):
        ...
        logger.debug(
            "LuaParser initialized (builtins=%s)",
            list(builtins.keys()),
        )
    ...
    def visit(self, node: astnodes.Node) -> Any:
        ...
        if logger.isEnabledFor(logging.DEBUG):
            logger.debug("visit dispatch: %s", node.display_name)
    ...
    def generic_visit(self, node: astnodes.Node) -> Any:
        ...
        logger.error("No visit handler for node type: %s", name)
    ...
    def visit_Call(self, node: astnodes.Call):
        ...
        # 未考虑参数收集和参数展开
        if logger.isEnabledFor(logging.DEBUG):
            func_repr = getattr(node.func, "id", None)
            if func_repr is None:
                func_repr = getattr(node.func, "name", "<func>")
            logger.debug("Call func=%s", func_repr)
            logger.debug("callable=%r", callable_)
    ...
    def lua_call(self, func_node, node):
        ...
        fname = getattr(getattr(func_node, "name", None), "id", "<anon>")
        if logger.isEnabledFor(logging.DEBUG):
            logger.debug("Calling Lua function %s", fname)
        ...
        self.return_value = None
        if logger.isEnabledFor(logging.DEBUG):
            logger.debug("Lua function %s returned %r", fname, result)
        return result

    def python_call(self, callable_, node):
        args = tuple(map(self.visit, node.args))
        if logger.isEnabledFor(logging.DEBUG):
            logger.debug("Calling python callable %r", callable_)
...
class Scope(dict):
    ...
    def __enter__(self):
        ...
        if logger.isEnabledFor(logging.DEBUG):
            logger.debug("Enter scope: %r", self)
            logger.debug("depth=%s enclosure=%s", self.depth, self.enclosure)
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if logger.isEnabledFor(logging.DEBUG):
            logger.debug("Exit scope: %r (exc_type=%r)", self, exc_type)
    ...
    def get_value(self, name: str):
        ...
        while scope is not None:
            ...
            if name in scope:
                if logger.isEnabledFor(logging.DEBUG):
                    logger.debug("Found %s in scope %r", name, scope)
                return scope[name]
            scope = scope.parent
            # print(f"Jump to {scope}")
        else:
            logger.debug("Identifier %s not found", name)
            logger.debug("starting from %r", self)
    ...
    def get_scope_by_name(self, name: str):
        ...
        while scope is not None:
            if name in scope:
                if logger.isEnabledFor(logging.DEBUG):
                    logger.debug("get_scope_by_name: found %s", name)
                    logger.debug("in %r", scope)
                return scope
            scope = scope.parent
        else:
            raise IdentifierNotFound(f"Not Found identifier: {name}")
    ...
    def recursive_print(self):
        logger.debug(f"Scope {self.depth}:")
        pprint(self)
        if self.parent:
