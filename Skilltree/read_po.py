...
def _scan_po(
    ...
    try:
        ...
        logger.debug("zip 中未找到文件: scripts/languages/%s", po_name)
    except Exception as e:
        logger.exception("解析 %s 时出错: %s", po_name, e)
...
def get_names(
    ...
    for msgctxt, data in po_dict.items():
        ...
        if msgid is not None and msgid != "":
            ...
            if msgid == msgstr:
                logger.debug(
                    "中英文重名 id:%s, msgid:%s, msgstr:%s", id_, msgid, msgstr
                )
                continue
            if msgstr == "":
                logger.debug("未翻译词条 id:%s, msgid:%s", id_, msgid)
                continue
        else:
            # 无英文名，跳过后续的中文名处理
            continue
    ...
    for msgctxt, data in scan_chn_po(include_strings).items():
        if msgctxt.startswith(start):
            ...
            if data["msgid"] and data["msgstr"] == "":
                logger.info(
                    "未翻译台词: %s, msgid: %s", msgctxt, data["msgid"]
                )
    return res
